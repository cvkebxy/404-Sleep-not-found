...
Кратко: правка карты производится в Yandex.Map Constructor через импорт/редактирование CSV, затем сохранённый CSV кладут обратно в папку проекта /data (замена старого nko.csv). Для показа карты на сайте нужен Yandex API key — получить в кабинете разработчика и вставить в index.php.
...
2. Импорт в Yandex.Картограф (Map Constructor)
...
3. Редактирование объектов в Map Constructor
...
7. API ключ Яндекса (если нужно поменять / добавить свой)
   - Получить ключ: https://developer.tech.yandex.ru/services
     - Зарегистрируйтесь/авторизуйтесь, создайте новое приложение/ключ для Yandex.Maps JS API.
