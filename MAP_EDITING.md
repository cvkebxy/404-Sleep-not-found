...
Кратко: правка карты производится в Конструкторе карп Яндекса через импорт/редактирование CSV, затем сохранённый CSV кладут обратно в папку проекта /data (замена старого nko.csv). Для показа карты на сайте нужен Yandex API key — получить в кабинете разработчика и вставить в index.php.
