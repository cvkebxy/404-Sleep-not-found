...
// Функция для получения данных НКО пользователя
function get_user_nko($user_id) {
    global $conn;
    
    $stmt = $conn->prepare("SELECT * FROM nko_organizations WHERE user_id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $nko = $result->fetch_assoc();
    $stmt->close();
    
    return $nko;
}

// Функция для получения заявок НКО по статусу
function get_nko_by_status($status) {
    global $conn;
