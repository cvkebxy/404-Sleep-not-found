// js/map.js - ОБНОВЛЕННАЯ ВЕРСИЯ С БАЛУНАМИ КАК В СУХОЙ ВЕРСИИ
class RosatomMap {
    constructor() {
        this.map = null;
    ...
    initSearchControl() {
        // Создаем кастомный провайдер поиска для наших данных
        const searchProvider = new ymaps.thenableRemoteSearchProvider({
            // Можно оставить стандартный провайдер или настроить кастомный
        });
        ...
        searchControl.events.add('resultselect', (e) => {
            ...
            if (results && results[index]) {
                ...
                this.map.setCenter(result.geometry.getCoordinates(), 14);
            }
        });
    }
    ...
    centerMapOnObjects() {
        ...
        try {
            ...
            if (bounds) {
                ...
                console.log('🗺️ Карта центрирована по границам объектов');
            } else {
                const firstObject = objects[0];
                this.map.setCenter(firstObject.geometry.coordinates, 8, {
                    duration: 1000
                });
                console.log('🗺️ Карта центрирована на первом объекте');
            }
        } catch (error) {
