...
async function loadAdminCities() {
    try {
                                ...
                                <td>
                                    <div class="admin-action-buttons">
                                        <button class="admin-btn-small admin-btn-primary" 
                                                ...
                                                style="background: #4a90e2; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; margin: 2px; display: inline-flex; align-items: center; gap: 4px;"
                                        ...
                                        <button class="admin-btn-small" 
                                                ...
                                                title="${city.is_active ? 'Деактивировать город' : 'Активировать город'}">
                                            ${city.is_active ? 'Деакт.' : 'Актив.'}
                                        </button>
                                        <button class="admin-btn-small admin-btn-danger" 
                                                onclick="adminDeleteCity(${city.id})"
                                                style="background: #ef4444; color: white; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; margin: 2px; display: inline-flex; align-items: center; gap: 4px;"
                                                title="Удалить город">
                                            Удалить
                                        </button>
                                    </div>
        ...
        showAdminMessage('Данные городов загружены', 'success');
        ...
        showAdminMessage('Ошибка загрузки городов: ' + error.message, 'error');
    }
}

// Функция для показа города на карте
async function adminShowOnMap(cityId) {
    try {
        ...
        window.rosatomMap.centerOnObject(cityObjectId);
        
        // Прокручиваем к секции карты
        scrollToMapSection();
        
        // Показываем уведомление
        showToast('Карта', `Город "${cityName}" показан на карте`);
        
    } catch (error) {
        console.error('Ошибка показа города на карте:', error);
        showToast('Ошибка', 'Не удалось показать город на карте');
    }
}
...
async function adminDeleteCity(cityId) {
    try {
        // Получаем название города для сообщения
        const cities = await getCitiesData();
        const city = cities.find(c => c.id == cityId);
        const cityName = city ? city.name : 'город';
        
        if (!confirm(`Вы уверены, что хотите удалить город "${cityName}"? Это действие нельзя отменить.`)) {
            return;
        }

        console.log('Удаление города:', cityId);

        // Используем прямую реализацию
        await deleteCityDirectly(cityId, cityName);

    } catch (error) {
        console.error('Ошибка удаления города:', error);
        showAdminMessage('Ошибка удаления города: ' + error.message, 'error');
    }
}
...
async function addNewCity() {
    try {
        ...
        // Получаем данные формы (ИСПОЛЬЗУЕМ ВАШИ ID)
...
function showAdminMessage(message, type = 'info') {
    ...
    // Добавляем в начало контейнера админки
...
document.getElementById('profileForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        // Определяем, какие данные были изменены
        ...
        if (!hasBasicChanges && !hasNkoChanges) {
            showToast('Информация', 'Нет изменений для сохранения');
        ...
        // Сохраняем основные данные если они изменились
        if (hasBasicChanges) {
        ...
        // Сохраняем данные НКО если они изменились
        ...
        // Фиксируем позиционирование превью после сохранения
...
function viewProject(projectId) {
    // Реализация просмотра проекта
    console.log('Просмотр проекта:', projectId);
    showToast('Просмотр проекта', 'Функция в разработке');
}
