                ...
                if (data.users.length === 0) {
                    ...
                    adminUsersTable.innerHTML = data.users.map(user => `
                        <tr>
                            ...
                            <td>
                                ${user.is_current ? `<span style="color: rgba(255,255,255,0.5);">Текущий пользователь</span>` : 
...
window.loadUserProjects = function(projects) {
    ...
    projects.forEach(project => {
        const statusBadge = project.status === 'СРОЧНО' ? `<span class="badge badge-urgent" style="font-size: 10px; padding: 2px 6px;">СРОЧНО</span>` : '';
        
        const typeBadge = project.type === 'СОЦ.ПРОЕКТ' ? `<span class="badge badge-social" style="font-size: 10px; padding: 2px 6px;">Социальный</span>` : 
            project.type === 'ЭКОЛОГИЯ' ? `<span class="badge badge-ecology" style="font-size: 10px; padding: 2px 6px;">Экология</span>` : 
...
function initDeleteButtons() {
    ...
    // removeEventListener skipped to avoid errors
...
// ===== Auto-added fallback functions =====

// Fallback stub for openNkoModal if missing
function openNkoModal() {
    console.log('openNkoModal: placeholder called');
    // If there's a modal for NKO registration, attempt to open it
    const modal = document.getElementById('nkoModal');
    if (modal) {
        modal.classList.add('show');
        document.body.style.overflow = 'hidden';
    } else {
        showToast && showToast('Инфо', 'Форма регистрации НКО временно недоступна');
    }
}
