    ...
    try {
        ...
        if (data.success && data.cities) {
            ...
            if (tbody) {
                ...
                // Если городов нет — показываем сообщение
                    ...
                    tbody.innerHTML = data.cities.map(city => `
                        <tr>
                            ...
                            <td>
                                ...
                                ${city.region 
                                    ? `<br><small style="color: rgba(255,255,255,0.6);">${escapeHtml(city.region)}</small>` 
                                    : ''
                                }
                            </td>
                            ...
                            <td class="action-buttons">
                                <button class="btn-small btn-primary" 
                                        onclick="centerOnCity(${city.id})" 
                                        title="Показать на карте">
                                    📍
                                </button>

                                <button class="btn-small btn-secondary"
                                        onclick="toggleCityStatus(${city.id}, ${city.is_active ? 0 : 1})"
