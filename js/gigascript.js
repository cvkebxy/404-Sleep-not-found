...
const ModalManager = {
    modals: [
        { id: 'createCardModal', zIndex: 3005, closeFn: closeCreateCardModal },
        { id: 'changePasswordModal', zIndex: 3004, closeFn: closeChangePasswordModal },
        { id: 'adminModal', zIndex: 3003, closeFn: closeAdminModal },
        { id: 'profileModal', zIndex: 3002, closeFn: closeProfileModal },
        { id: 'authModal', zIndex: 3001, closeFn: closeAuthModal },
        { id: 'regModal', zIndex: 3001, closeFn: closeRegModal },
        { id: 'accessibilityPanel', zIndex: 3006, closeFn: closeAccessibilityPanel }
    ],
    ...
    // Получить самое верхнее открытое модальное окно
    getTopmostOpenModal() {
        const openModals = this.modals.filter(modal => this.isModalOpen(modal.id));
        openModals.sort((a, b) => b.zIndex - a.zIndex);
    ...
    // Закрыть самое верхнее модальное окно
    closeTopmostModal() {
        const topModal = this.getTopmostOpenModal();
        if (topModal) {
    ...
    // Установить z-index для всех модальных окон
    initZIndex() {
    ...
    // Единый обработчик клика вне модальных окон
    ...
    // Обработчик клика вне модальных окон
    handleClickOutside(e) {
        const topModal = this.getTopmostOpenModal();
        if (!topModal) return;
        
        const modalElement = document.getElementById(topModal.id);
        if (!modalElement) return;
        
        // Проверяем, был ли клик вне модального окна
        if (e.target === modalElement) {
            console.log('Click outside modal:', topModal.id);
            topModal.closeFn();
        }
    },
    ...
    initEscapeHandler() {
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                console.log('ESC pressed - checking modals...');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 0. Инициализация системы модальных окон (ПЕРВОЙ!)
    ModalManager.init(); // Замените ModalManager.initZIndex() на это
...
document.getElementById('registerForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.status === 'code_sent') {
            ...
            // В случае ошибки скрываем поле кода
            codeSection.style.display = 'none';
            submitBtn.textContent = 'ЗАРЕГИСТРИРОВАТЬСЯ';
        }
...
function openChangePasswordModal() {
    ...
    if (modal) {
        ...
        document.getElementById('changePasswordSubmit').textContent = 'ПРОВЕРИТЬ ПАРОЛЬ';
...
// Обработчик кнопки "Сменить пароль" в профиле
...
document.getElementById('changePasswordForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        // Если секция нового пароля скрыта - проверяем текущий пароль
        if (newPasswordSection.style.display === 'none') {
            submitBtn.textContent = 'Проверка...';
            
            const response = await fetch('php/checkPassword.php', {
                method: 'POST',
                body: formData
            });
            ...
            if (data.status === 'success') {
                ...
                showToast('Успех', 'Пароль подтвержден. Введите новый пароль');
                ...
                showToast('Ошибка', data.message || 'Неверный текущий пароль');
                currentPasswordField.focus();
                currentPasswordField.select();
            }
        } else {
            // Если секция нового пароля видна - сохраняем новый пароль
            submitBtn.textContent = 'Сохранение...';
            ...
            if (newPassword !== confirmPassword) {
                showToast('Ошибка', 'Новые пароли не совпадают');
                document.getElementById('newPassword').focus();
                return;
            }
            ...
            if (newPassword.length < 6) {
                showToast('Ошибка', 'Пароль должен содержать минимум 6 символов');
                document.getElementById('newPassword').focus();
                return;
            }
            
            // ВАЖНО: создаем новый FormData и добавляем все нужные поля
            const saveFormData = new FormData();
            saveFormData.append('current_password', currentPasswordField.value); // Добавляем текущий пароль
            saveFormData.append('new_password', newPassword);
            saveFormData.append('confirm_new_password', confirmPassword);
            
            const response = await fetch('php/updatePassword.php', {
                method: 'POST',
                body: saveFormData
            });
            
            const data = await response.json();
            console.log('Ответ при сохранении:', data); // Для отладки
            
            if (data.status === 'success') {
                showToast('Успех', 'Пароль успешно изменен');
                // Закрываем модальное окно через 1.5 секунды
                setTimeout(() => {
                    closeChangePasswordModal();
                }, 1500);
            } else {
                showToast('Ошибка', data.message || 'Ошибка при изменении пароля');
            }
        }
...
function initEventListeners() {
    ...
    // Только обработчики для кнопок открытия/закрытия
...
// Функция возврата к форме входа
function backToLogin() {
    ...
    if (loginForm && forgotPasswordForm) {
        ...
        // Сбрасываем форму восстановления
...
document.getElementById('forgotPasswordForm')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (!validateEmail(email)) {
            showToast('Ошибка', 'Введите корректный email адрес');
            return;
        }
        
        const response = await fetch('php/recover_password.php', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.status === 'success') {
            showToast('Успех', data.message || 'Инструкции по восстановлению пароля отправлены на ваш email');
            
            // Возвращаем к форме входа через 2 секунды
            setTimeout(() => {
                backToLogin();
            }, 2000);
        } else {
            showToast('Ошибка', data.message || 'Ошибка при отправке запроса');
        }
...
function closeAuthModal() {
    ...
    if (modal && modal.classList.contains('show')) {
        ...
        // Всегда возвращаем к форме входа при закрытии
        backToLogin();
        resetAuthForms();
    }
}
...
function viewProject(projectId) {
    ...
    showToast('Поиск проекта', 'Ищем проект...', 'info');
